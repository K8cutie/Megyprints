  ...
  return (
    <div className="min-h-screen bg-cream pt-24 pb-12">
      ...
      <div className="text-center px-6 py-16">
        <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="font-display text-5xl font-bold text-dark">We're Megy Prints</motion.h1>
        <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
          className="mt-4 text-lg text-medium max-w-[600px] mx-auto">A family-owned printing business dedicated to turning your precious memories into beautiful keepsakes you'll treasure forever.</motion.p>
      </div>
      ...
      <div className="max-w-[1000px] mx-auto px-6 grid md:grid-cols-3 gap-6 mb-16">
        {values.map((v, i) => (
          <motion.div key={v.title} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 * i }}
            ...
            <h3 className="font-display text-lg font-semibold text-dark">{v.title}</h3>
            <p className="text-sm text-medium mt-2">{v.desc}</p>
          </motion.div>
        ))}
      </div>
      ...
      <div className="max-w-[800px] mx-auto px-6 mb-16">
        <h2 className="font-display text-3xl font-bold text-center text-dark mb-8">How It Works</h2>
        <div className="flex items-center justify-between">
          {steps.map((step, i) => (
            <div key={step} className="flex items-center">
              <div className="text-center">
                <div className="w-12 h-12 rounded-full bg-blush flex items-center justify-center text-blush-pink font-display font-bold text-lg mx-auto">{i + 1}</div>
                <p className="text-xs font-medium text-dark mt-2">{step}</p>
              </div>
              {i < steps.length - 1 && <div className="w-12 sm:w-20 h-px bg-peach mx-1 sm:mx-2" />}
            </div>
          ))}
        </div>
      </div>
