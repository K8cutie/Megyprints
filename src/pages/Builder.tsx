...
import {
  Settings, Upload, LayoutTemplate, PenTool, Eye, ChevronRight, Sparkles,
} from 'lucide-react';
...
export default function Builder() {
  ...
  const handleGenerate = useCallback(() => {
    actions.generateAlbumPages();
    actions.setPhase('edit');
  }, [actions]);

  const handleRegenerate = useCallback(() => {
    actions.regenerateAlbumPages();
  }, [actions]);
        ...
        <div className="h-12 bg-white border-b border-[#E8E8E8] flex items-center px-4 gap-1 shrink-0">
          ...
          {actions.phase === 'edit' && (
            <button
              ...
              className="px-3 py-1 text-xs font-medium text-[#6B6B6B] border border-[#D4D4D4] rounded-lg hover:bg-[#FDE8E4] hover:text-[#E8A598] flex items-center gap-1 transition-all"
            >
            ...
            {actions.phase === 'template' && (
              <motion.div key="template" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }}
                ...
                <BuilderTemplate
                  ...
                  hasPages={actions.albumPages.length > 0 && actions.albumPages[0].photos.length > 0}
                />
              </motion.div>
            )}
