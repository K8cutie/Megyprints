...
export default function MessagesPanel() {
  ...
  if (loading) {
    return <div className="py-16 text-center"><Loader2 size={20} className="animate-spin inline text-light" /></div>;
  }
    ...
    <div className="max-w-3xl">
      <div className="flex items-center justify-between mb-4">
        ...
        <button onClick={() => { setLoading(true); void load(); }}
          className="h-9 px-3 rounded-lg border border-line text-sm flex items-center gap-1.5 hover:border-peach">
      ...
      {rows.length === 0 && !err && (
        <div className="py-16 text-center text-sm text-light">
      ...
      <div className="space-y-3">
        {rows.map((m) => (
          <div key={m.id} className="rounded-xl border border-[#EEE7DE] bg-white p-4">
            ...
            <div className="mt-0.5 text-sm text-medium">
              {m.name}
              {' · '}
              {/* Rendered as text, never as markup — this is anonymous public input. */}
              <a href={`mailto:${encodeURIComponent(m.email)}`} className="text-rust hover:underline">
                {m.email}
              </a>
            </div>
            <p className="mt-2 text-sm text-dark whitespace-pre-wrap break-words">{m.message}</p>
          </div>
        ))}
      </div>
