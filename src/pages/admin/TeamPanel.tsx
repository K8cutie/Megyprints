    ...
    <div className="max-w-2xl">
      ...
      <div className="flex flex-wrap items-center gap-2 mb-2">
        <input value={email} onChange={(e) => setEmail(e.target.value)} type="email" placeholder="teammate@email.com"
          className="h-9 px-3 rounded-lg border border-line text-sm outline-none focus:border-peach flex-1 min-w-[200px]" />
        <select value={role} onChange={(e) => setRoleSel(e.target.value as Role)}
          className="h-9 px-2 rounded-lg border border-line text-sm bg-white">
          <option value="fulfillment">Fulfillment</option>
          <option value="owner">Owner</option>
        </select>
        <button onClick={add} disabled={busy}
          className="h-9 px-4 rounded-lg bg-peach text-white text-sm font-semibold flex items-center gap-1.5 disabled:opacity-50">
          {busy ? <Loader2 size={14} className="animate-spin" /> : <UserPlus size={14} />} Add
        </button>
      </div>
      <p className="text-xs text-light mb-5">The teammate must sign up with this email before they can log in.</p>
      {err && <p className="text-xs text-red-600 mb-3">{err}</p>}

      {/* Built-in owners */}
      <h3 className="text-xs font-semibold text-light uppercase tracking-wide mb-2">Owners</h3>
      <div className="rounded-xl border border-line bg-white divide-y divide-line-soft mb-5">
        {ADMIN_EMAILS.map((em) => (
          <div key={em} className="flex items-center justify-between px-4 py-2.5">
            <span className="text-sm text-dark">{em}</span>
            <span className="text-xs px-2 py-0.5 rounded-full bg-blush text-blush-pink font-semibold">Owner · built-in</span>
          </div>
        ))}
      </div>

      {/* Assigned teammates */}
      <h3 className="text-xs font-semibold text-light uppercase tracking-wide mb-2">Team</h3>
      {loading ? (
        <div className="py-10 flex justify-center text-light"><Loader2 className="w-5 h-5 animate-spin" /></div>
      ) : rows.length === 0 ? (
        <p className="text-sm text-light text-center py-6">No teammates added yet.</p>
      ) : (
        <div className="rounded-xl border border-line bg-white divide-y divide-line-soft">
          {rows.map((r) => (
            <div key={r.email} className="flex items-center justify-between px-4 py-2.5">
              <span className="text-sm text-dark">{r.email}</span>
              <div className="flex items-center gap-3">
                <span className="text-xs px-2 py-0.5 rounded-full bg-line-soft text-medium font-medium capitalize">{r.role}</span>
