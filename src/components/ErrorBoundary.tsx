    ...
    if (this.state.hasError) {
      return (
        <div className="flex min-h-screen flex-col items-center justify-center bg-cream px-6 text-center">
          ...
          <p className="mt-2 max-w-sm text-sm text-medium">
          ...
          {import.meta.env.DEV && this.state.message && (
            <code className="mt-3 max-w-md break-words rounded-lg bg-black/5 px-3 py-2 text-xs text-medium">
              {this.state.message}
            </code>
          )}
          <button
            onClick={() => window.location.reload()}
            className="mt-6 rounded-lg bg-peach px-6 py-2.5 font-body text-sm font-semibold text-white transition-all hover:brightness-105"
          >
            Reload
          </button>
        </div>
