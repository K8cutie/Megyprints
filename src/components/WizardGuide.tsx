    ...
    <>
      ...
      {!isVisible && !isGloballyDismissed() && (
        <motion.button
          ...
          onClick={handleShow}
          className="fixed bottom-6 right-6 z-50 pl-3 pr-4 py-2.5 rounded-full bg-peach text-white shadow-lg hover:brightness-110 transition-all flex items-center gap-2"
          title="Show Guide"
        >
            ...
            >
              <div className="bg-white rounded-3xl shadow-2xl overflow-hidden">
                ...
                <div className="p-6 pb-4">
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-[0.7rem] font-medium text-taupe uppercase tracking-wider">
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-lg text-taupe hover:text-ink-warm hover:bg-sand transition-colors"
                    >
                      <X size={16} />
                    </button>
                  </div>
                ...
                <div className="px-6 pb-2">
                  <p className="text-ink-warm text-sm leading-relaxed">
                    {currentStep.body}
                  </p>
                  ...
                  {currentStep.tips && currentStep.tips.length > 0 && (
                    <div className="mt-4">
                      <button
                        ...
                        className="flex items-center gap-1.5 text-xs font-medium text-blush-pink hover:text-coral-soft transition-colors"
                      >
                        <Lightbulb size={12} />
                        ...
                        {showTips && (
                          ...
                          >
                            {currentStep.tips.map((tip, i) => (
                              <li
                                key={i}
                                className="flex items-start gap-2 text-xs text-medium"
                              >
                    ...
                    {currentStep.id === 'upload-photos' && (
                      <>
                        <button
                          ...
                          className="flex items-center gap-2 px-4 py-2 bg-peach text-white text-sm font-medium rounded-lg hover:brightness-105 transition-all"
                        >
                          <Upload size={14} />
                          Browse Files
                        </button>
                    ...
                    {currentStep.id === 'photo-count' && (
                      <div className="flex flex-col gap-2 mt-3 w-full">
                        {[
                          ...
                          <label
                            key={opt.value}
                            className="flex items-center gap-3 px-3 py-2.5 rounded-lg border border-blush-deep hover:border-peach hover:bg-blush/30 cursor-pointer transition-all"
                          >
                            <input
                              type="radio"
                              name="photos-per-page"
                              className="accent-peach w-4 h-4 shrink-0"
                    ...
                    {currentStep.id === 'generate-layouts' && (
                      <>
                        <button
                          ...
                          className="flex items-center gap-2 px-4 py-2 bg-soft-lavender text-white text-sm font-medium rounded-lg hover:brightness-105 transition-all"
                        >
                          <Sparkles size={14} />
                          Generate All
                        </button>
                        <button
                          ...
                          className="flex items-center gap-2 px-4 py-2 border border-soft-lavender text-soft-lavender text-sm font-medium rounded-lg hover:bg-soft-lavender/5 transition-all"
                        >
                          <Sparkles size={14} />
                          Generate This Page
                        </button>
                    ...
                    {currentStep.id === 'add-background' && (
                      <>
                        <button
                          ...
                          className="flex items-center gap-2 px-4 py-2 bg-mint text-white text-sm font-medium rounded-lg hover:brightness-105 transition-all"
                        >
                          <Palette size={14} />
                          Open Background Tab
                        </button>
                    ...
                    {currentStep.id === 'add-text' && (
                      <>
                        <button
                          ...
                          className="flex items-center gap-2 px-4 py-2 bg-sky-blue text-white text-sm font-medium rounded-lg hover:brightness-105 transition-all"
                        >
                          <Type size={14} />
                          Add Text
                        </button>
                    ...
                    {currentStep.id === 'photo-panning' && (
                      <div className="flex flex-col gap-3 mt-3 w-full">
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          {[
                            { step: '1', text: 'Click photo to select', color: 'bg-peach' },
                            { step: '2', text: 'Click "Containers" button', color: 'bg-[#3B82F6]' },
                            { step: '3', text: 'Drag photo inside frame', color: 'bg-mint' },
                            { step: '4', text: 'Click outside → Done!', color: 'bg-soft-lavender' },
                          ].map((s) => (
                            <div key={s.step} className={`${s.color}/10 border ${s.color.replace('bg-', 'border-')}/30 rounded-lg p-2.5 flex items-start gap-2`}>
                              ...
                              <span className="text-xs text-ink-warm font-medium leading-tight">{s.text}</span>
                            </div>
                          ))}
                        </div>
                        {/* Action buttons */}
                        ...
                        <button
                          ...
                          className="text-xs text-taupe hover:text-ink-warm transition-colors text-center"
                        >
                          Done, let me try it
                        </button>
                      </div>
                ...
                <div className="p-6 pt-4 border-t border-sand">
                  {/* Progress bar */}
                  ...
                  <div className="flex items-center justify-between">
                    <button
                      ...
                      className="text-xs text-taupe hover:text-ink-warm transition-colors"
                    >
                      Don&apos;t show again
                    </button>
                    ...
                    <div className="flex items-center gap-2">
                      {canGoBack && (
                        <button
                          onClick={handleBack}
                          className="flex items-center gap-1 px-3 py-2 text-sm text-medium hover:bg-sand rounded-lg transition-colors"
                        >
                          <ChevronLeft size={14} />
                          Back
                        </button>
                      ...
                      <button
                        onClick={handleNext}
                        className="flex items-center gap-1 px-5 py-2 bg-peach text-white text-sm font-semibold rounded-lg hover:brightness-105 transition-all"
                      >
